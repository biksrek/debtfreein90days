...
const required=['RAZORPAY_KEY_ID','RAZORPAY_KEY_SECRET','PRODUCT_PRICE_USD'];
...
const currency='USD';
const priceSubunit=()=>Math.round(Number(process.env.PRODUCT_PRICE_USD)*100);

app.post('/api/create-order',async(req,res)=>{
  try{
    if(req.body?.product!=='debt-free-in-90-days') return res.status(400).json({error:'Invalid product.'});
    const amount=priceSubunit();
    if(!amount||amount<1) return res.status(500).json({error:'Product price is not configured.'});
    const order=await razorpay.orders.create({amount,currency,receipt:`df90_${Date.now()}`,notes:{product:'debt-free-in-90-days'}});
...
app.post('/api/verify-payment',async(req,res)=>{
  try{
    ...
    if(!['created','attempted','paid'].includes(order.status))return res.status(400).json({error:'Invalid Razorpay order.'});
    if(order.amount!==priceSubunit()||order.currency!==currency||order.notes?.product!=='debt-free-in-90-days')return res.status(400).json({error:'Order does not match this product.'});
    ...
    if(payment.order_id!==razorpay_order_id||payment.status!=='captured'||payment.currency!==currency||payment.amount!==priceSubunit())return res.status(400).json({error:'Payment has not been captured for the expected product amount.'});
...
app.get('/api/downloads',async(req,res)=>{
  try{
    ...
    if(order.status!=='paid'||order.amount!==priceSubunit()||order.currency!==currency||order.notes?.product!=='debt-free-in-90-days'||payment.order_id!==order_id||payment.status!=='captured'||payment.amount!==priceSubunit()||payment.currency!==currency)return res.status(403).json({error:'Purchase could not be verified.'});
