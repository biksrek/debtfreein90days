...
// Keep webhook raw-body handling before the normal JSON parser.
app.post('/api/razorpay-webhook', express.raw({type:'application/json'}), (req,res) => {
  try {
    ...
    // In production, enqueue event.id and process it idempotently in a durable DB/queue.
...
function makeToken(orderId,paymentId) {
...
function parseToken(token,orderId,paymentId) {
  const decoded = Buffer.from(token,'base64url').toString('utf8');
  const parts = decoded.split('.');
  if (parts.length !== 4 || parts[0]!==orderId || parts[1]!==paymentId || Number(parts[2]) < Date.now()) return false;
  const expected = crypto.createHmac('sha256',tokenSecret).update(`${parts[0]}.${parts[1]}.${parts[2]}`).digest('hex');
...
app.post('/api/verify-payment', async (req,res) => {
  try {
    ...
    // Verify using the server-created order id and server-side secret.
    ...
    const expectedAmount = pricePaise();
    if (payment.order_id !== razorpay_order_id || payment.status !== 'captured' || payment.currency !== 'INR' || payment.amount !== expectedAmount) {
      return res.status(400).json({error:'Payment has not been captured for the expected product amount.'});
    }
...
app.get('/api/downloads', async (req,res) => {
  try {
    ...
    if (payment.order_id !== order_id || payment.status !== 'captured' || payment.amount !== pricePaise()) return res.status(403).json({error:'Purchase could not be verified.'});
    const files = [
      {label:'Normal PDF',url:process.env.DOWNLOAD_NORMAL_PDF_URL},
      {label:'Mobile PDF',url:process.env.DOWNLOAD_MOBILE_PDF_URL},
      {label:'EPUB',url:process.env.DOWNLOAD_EPUB_URL}
    ].filter(x=>x.url);
