...
if (!emailInput && terms) {
  ...
  label.innerHTML = '<span>Email address for your receipt & secure download links</span>';
...
const validEmail = () => !!emailInput && emailInput.value.trim() !== '' && emailInput.checkValidity();
const updateButton = () => { if (buyButton) buyButton.disabled = !terms?.checked || !validEmail(); };
terms?.addEventListener('change', updateButton);
emailInput?.addEventListener('input', updateButton);
updateButton();

buyButton?.addEventListener('click', async () => {
  if (!terms?.checked) { setStatus('Please accept the no-refund digital product terms.'); return; }
  if (!validEmail()) { emailInput?.focus(); setStatus('Please enter a valid email address.'); return; }
  buyButton.disabled = true;
  setStatus('Creating your secure order…');
  try {
    const customerEmail = emailInput.value.trim().toLowerCase();
    const response = await fetch('/api/create-order', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({product: 'debt-free-in-90-days', email: customerEmail}) });
    ...
    const rzp = new Razorpay({
      ...
      name: 'Debt-Free in 90 Days', description: 'Debt-Free in 90 Days — Guide + 90-Day Toolkit', order_id: order.order_id,
      prefill: {name: '', email: customerEmail}, notes: {product: 'debt-free-in-90-days', customer_email: customerEmail},
      theme: {color: '#d8a92e'},
      ...
      handler: async function(payment) {
        ...
        const verifyResponse = await fetch('/api/verify-payment', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({...payment, customer_email: customerEmail}) });
